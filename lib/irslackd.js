...
const IRSLACKD_DELETED_USER = 'IRSLACKD_DELETED_';
...
class Irslackd {
  ...
  async resolveSlackUser(ircUser, slackUser) {
    ...
    if (user.user.deleted) {
      console.debug('user deleted: ' + ircNick + ' : ' + user.user.deleted);
      ircNick = IRSLACKD_DELETED_USER + ircNick;
    }
    ircUser.mapIrcToSlack(ircNick, slackUser);
    return ircNick;
  }
...
class IrcUser {
  ...
  joinChannel(ircChan, ircNicks) {
    ...
    ircNicks.forEach((ircNick) => {
      // bug #67
      if (!ircNick.startsWith(IRSLACKD_DELETED_USER)) {
        this.addNickToChannel(ircChan, ircNick);
      } else {
        console.log('not adding deleted user: ' + ircNick);
      }
    });
  }
  partChannel(ircChan) {
